...
// The maintained SDK exposes the caller stack through logos::detail rather
// than the newer test-only CallCaller helper. Keep the tests source-compatible
// with both SDK shapes while exercising the same production ambient identity.
class CallCaller {
public:
    static CallCaller module(const std::string& name)
    {
        return CallCaller("{\"kind\":\"module\",\"name\":\"" + name + "\"}");
    }

    static CallCaller host() { return CallCaller("{\"kind\":\"host\"}"); }
...
private:
    explicit CallCaller(const std::string& document) { detail::setCallCaller(document.c_str()); }
};
...
LOGOS_TEST(informModuleTokenScoped_keeps_bootstrap_tokens_per_instance) {
    ...
    CapabilityModuleImpl impl;
    LOGOS_ASSERT_TRUE(impl.informModuleTokenScoped(
        kTrustedToken, "lez_indexer_module", "zone_alpha", "alpha-bootstrap"));
    LOGOS_ASSERT_TRUE(impl.informModuleTokenScoped(
        kTrustedToken, "lez_indexer_module", "zone_beta", "beta-bootstrap"));

    LOGOS_ASSERT_EQ(logos::host::tokenFor(scopedTokenKey(
                          "lez_indexer_module", "zone_alpha")),
                    std::string("alpha-bootstrap"));
    LOGOS_ASSERT_EQ(logos::host::tokenFor(scopedTokenKey(
                          "lez_indexer_module", "zone_beta")),
                    std::string("beta-bootstrap"));
    LOGOS_ASSERT_TRUE(logos::host::tokenFor("lez_indexer_module").empty());
}
...
LOGOS_TEST(informModuleTokenScoped_rejects_untrusted_business_token) {
    ...
    CapabilityModuleImpl impl;
    LOGOS_ASSERT_FALSE(impl.informModuleTokenScoped(
        "seed-token-untrusted_module", "lez_indexer_module", "zone_alpha",
        "forged-bootstrap"));
    LOGOS_ASSERT_TRUE(logos::host::tokenFor(scopedTokenKey(
                          "lez_indexer_module", "zone_alpha"))
                          .empty());
}
...
LOGOS_TEST(requestModuleScoped_uses_scoped_bootstrap_without_default_fallback) {
    ...
    LOGOS_ASSERT_TRUE(impl.requestModuleScoped(
        "requester_module", "lez_indexer_module", "zone_alpha").empty());
    ...
    const std::string alphaToken = impl.requestModuleScoped(
        "requester_module", "lez_indexer_module", "zone_alpha");
    const std::string betaToken = impl.requestModuleScoped(
        "requester_module", "lez_indexer_module", "zone_beta");
    LOGOS_ASSERT(isUuid(alphaToken));
    LOGOS_ASSERT(isUuid(betaToken));
...
LOGOS_TEST(requestModuleScoped_rejects_invalid_identity_and_restrictions) {
    ...
    const auto allowedCaller = logos::CallCaller::module("allowed_caller");
    LOGOS_ASSERT_TRUE(impl.requestModuleScoped(
        "allowed_caller", "lez_indexer_module", "").empty());

    LOGOS_ASSERT_TRUE(impl.registerRestriction(
        kTrustedToken, "lez_indexer_module", {"allowed_caller"}));
    const auto blockedCaller = logos::CallCaller::module("blocked_caller");
    LOGOS_ASSERT_TRUE(impl.requestModuleScoped(
        "blocked_caller", "lez_indexer_module", "zone_alpha").empty());
    const auto allowedCallerAgain = logos::CallCaller::module("allowed_caller");
    LOGOS_ASSERT(isUuid(impl.requestModuleScoped(
        "allowed_caller", "lez_indexer_module", "zone_alpha")));
}
