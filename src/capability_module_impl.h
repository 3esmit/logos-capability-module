...
public:
    ...
    // Request a token for one explicit runtime instance. The instance key is
    // kept separate from the default module token so a default target cannot
    // satisfy an explicit-instance request.
    std::string requestModuleScoped(const std::string& fromModuleName,
                                    const std::string& moduleName,
                                    const std::string& instanceId);

    // Register the bootstrap token for one explicit runtime instance. Only
    // the trusted core/capability channel may populate this private keyspace.
    bool informModuleTokenScoped(const std::string& authToken,
                                 const std::string& moduleName,
                                 const std::string& instanceId,
